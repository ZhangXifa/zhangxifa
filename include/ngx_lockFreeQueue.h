...
/*
    在此项目中无锁编程相较于传统的锁机制的优势在于：
    消除上下文的开销：
        传统锁：线程阻塞时发生用户态->内核态的切换，开销巨大
        无锁机制：始终运行在用户态，CPU时间用于业务处理
    避免所竞争：
        传统锁：所有线程竞争同一个锁，串行话执行
        无锁机制：多个线程可并发操作，只在关键点竞争
    线性可伸缩性：
        随着CPU核心的增加，无锁性能接近线性提升
        传统锁性能受限于锁的串行化特性
